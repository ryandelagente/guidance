<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Test to Inventory</h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('psych-tests.store') }}" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required maxlength="200"
                               ...
                               placeholder="e.g. Otis-Lennon School Ability Test">
                    </div>
                        ...
                        <div>
                            ...
                            <select name="test_type" required class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                                ...
                                @foreach(['iq','personality','career_aptitude','interest','mental_health','other'] as $t)
                                    <option value="{{ $t }}" @selected(old('test_type') === $t)>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" name="total_items" value="{{ old('total_items') }}" min="1" max="9999"
                                   class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Publisher</label>
                            <input type="text" name="publisher" value="{{ old('publisher') }}" maxlength="200"
                                   class="w-full border-gray-300 rounded-md shadow-sm text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Edition Year</label>
                            <input type="number" name="edition_year" value="{{ old('edition_year') }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" maxlength="2000"
                                  class="w-full border-gray-300 rounded-md shadow-sm text-sm">{{ old('description') }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="is_active" class="text-sm font-medium text-gray-700">Active (available for scheduling)</label>
                    </div>
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <a href="{{ route('psych-tests.index') }}"
