<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('sessions.show', $session) }}" class="text-gray-400 hover:text-gray-600">← Back</a>
            <h2 class="font-semibold text-xl text-gray-800">Edit Case Notes</h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('sessions.update', $session) }}" class="space-y-5">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-lg p-4 text-sm text-blue-800">
                    <strong>{{ $session->studentProfile->full_name }}</strong>
                    · {{ $session->appointment->appointment_date->format('M d, Y') }}
                </div>
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg px-4 py-2 text-xs text-yellow-700">
                    🔒 Notes are re-encrypted on save.
                </div>

                <div class="bg-white shadow-sm rounded-lg p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Session Status</label>
                            <select name="session_status" class="w-full border-gray-300 rounded-md text-sm">
                                @foreach(['initial'=>'Initial','ongoing'=>'Ongoing','terminated'=>'Terminated','referred'=>'Referred'] as $val => $label)
                                    <option value="{{ $val }}" @selected($session->session_status === $val)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="presenting_concern" class="w-full border-gray-300 rounded-md text-sm">
                                ...
                                @foreach(['academic','personal_social','career','financial','family','mental_health','behavioral','other'] as $c)
                                    <option value="{{ $c }}" @selected($session->presenting_concern === $c)>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Case Notes (Confidential)</label>
                        <textarea name="case_notes" rows="10"
                                  class="w-full border-gray-300 rounded-md text-sm font-mono">{{ old('case_notes', $session->case_notes) }}</textarea>
                    </div>
                    <div>
                ...
                <div class="flex justify-end gap-3">
                    <a href="{{ route('sessions.show', $session) }}" class="px-4 py-2 text-sm text-gray-600">Cancel</a>
